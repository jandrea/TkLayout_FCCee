//Edited in 2021 by N. Lorber (equally spaced discs)
...
void Endcap::build()
{
  try {
    ...
    double         alpha          = (outerZ()-innerZ())/(numDisks()-1); // edited for FCCee by NL
    ...
    for (int i = 1; i <= numDisks(); i++) {
      ...
      // Apply correct offset for each disc versus middle position (edited for equally spaced discs)
      double zOffset = innerZ() + alpha * (i-1);
