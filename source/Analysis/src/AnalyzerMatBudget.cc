 ...
 *
 *  Edited in 2021 by N. Lorber (switch from pseudorapidity eta to polar angle theta)
 */
...
bool AnalyzerMatBudget::init(int nMatTracks)
{
  ...
  //c'est sale
  double etaMax_tmp = etaMax;
  double etaMin_tmp = etaMin;

etaMax = 2*atan(exp(-1*etaMin_tmp));
etaMin = 2*atan(exp(-1*etaMax_tmp));
  ...
  if (m_nTracks<=0 || m_trackers.size()==0) {
    ...
    for (int iTrk=0; iTrk<=m_trackers.size(); iTrk++) {
      ...
      m_radMB[trkName]["Barrel"].SetBins(m_nBins, 0, p4);
      ...
      m_intMB[trkName]["Barrel"].SetBins(m_nBins, 0, p4);
      ...
      m_radMB[trkName]["Endcap"].SetBins(m_nBins, 0, p4);
      ...
      m_intMB[trkName]["Endcap"].SetBins(m_nBins, 0, p4);
      ...
      m_radMB[trkName]["Services"].SetBins(m_nBins, 0, p4);
      ...
      m_intMB[trkName]["Services"].SetBins(m_nBins, 0, p4);
      ...
      m_radMB[trkName]["Supports"].SetBins(m_nBins, 0, p4);
      ...
      m_intMB[trkName]["Supports"].SetBins(m_nBins, 0, p4);
      ...
      m_radMB[trkName]["Total"].SetBins(m_nBins, 0, p4);
      ...
      m_intMB[trkName]["Total"].SetBins(m_nBins, 0, p4);
...
bool AnalyzerMatBudget::analyze()
{
  ...
  for (auto iTracker : m_trackers) {
    ...
    for (auto iTrack=0; iTrack<m_nTracks; iTrack++) {
      ...
      //c'est sale
      double theta = 0 + p2/m_nTracks*(iTrack+0.5);
      double eta = theta;
      ...
      double t2 = radtodeg(theta);

      // Study beam pipe -> fill only once
      if (iTracker==*m_trackers.begin()) {

        m_radMB["Beampipe"]["Total"].Fill(t2, m_beamPipe->radLength()/sin(theta));
        m_intMB["Beampipe"]["Total"].Fill(t2, m_beamPipe->intLength()/sin(theta));

        m_radMB["Tracker"]["Total"].Fill(t2, m_beamPipe->radLength()/sin(theta));
        m_intMB["Tracker"]["Total"].Fill(t2, m_beamPipe->intLength()/sin(theta));
      }
      ...
      // Given sub-tracker
      m_radMB[trkName]["Barrel"].Fill(t2, matBudget["Barrel"].radiation);
      m_intMB[trkName]["Barrel"].Fill(t2, matBudget["Barrel"].interaction);

      m_radMB[trkName]["Endcap"].Fill(t2, matBudget["Endcap"].radiation);
      m_intMB[trkName]["Endcap"].Fill(t2, matBudget["Endcap"].interaction);

      m_radMB[trkName]["Supports"].Fill(t2, matBudget["Supports"].radiation);
      m_intMB[trkName]["Supports"].Fill(t2, matBudget["Supports"].interaction);

      m_radMB[trkName]["Services"].Fill(t2, matBudget["Services"].radiation);
      m_intMB[trkName]["Services"].Fill(t2, matBudget["Services"].interaction);

      m_radMB[trkName]["Total"].Fill(t2, matBudget["Barrel"].radiation +
                                          matBudget["Endcap"].radiation +
                                          matBudget["Supports"].radiation +
                                          matBudget["Services"].radiation +
                                          m_beamPipe->radLength()/sin(theta));
      m_intMB[trkName]["Total"].Fill(t2, matBudget["Barrel"].interaction +
      ...
      m_radMB["Tracker"]["Barrel"].Fill(t2, matBudget["Barrel"].radiation);
      m_intMB["Tracker"]["Barrel"].Fill(t2, matBudget["Barrel"].interaction);

      m_radMB["Tracker"]["Endcap"].Fill(t2, matBudget["Endcap"].radiation);
      m_intMB["Tracker"]["Endcap"].Fill(t2, matBudget["Endcap"].interaction);

      m_radMB["Tracker"]["Supports"].Fill(t2, matBudget["Supports"].radiation);
      m_intMB["Tracker"]["Supports"].Fill(t2, matBudget["Supports"].interaction);

      m_radMB["Tracker"]["Services"].Fill(t2, matBudget["Services"].radiation);
      m_intMB["Tracker"]["Services"].Fill(t2, matBudget["Services"].interaction);

      m_radMB["Tracker"]["Total"].Fill(t2, matBudget["Barrel"].radiation +
                                            matBudget["Endcap"].radiation +
                                            matBudget["Supports"].radiation +
                                            matBudget["Services"].radiation);
      m_intMB["Tracker"]["Total"].Fill(t2, matBudget["Barrel"].interaction +
                                            matBudget["Endcap"].interaction +
                                            matBudget["Supports"].interaction +
                                            matBudget["Services"].interaction);
      ...
      for (auto iMap : matBudget) {
        ...
        if (compName!="Barrel" && compName!="Endcap") {
          ...
          if (m_radMBComp[trkName].find(compName)==m_radMBComp[trkName].end()) {
            ...
            m_radMBComp[trkName][compName].SetBins(m_nBins, 0, p4);
            ...
            m_intMBComp[trkName][compName].SetBins(m_nBins, 0, p4);
          }
          if (m_radMBComp["Tracker"].find(compName)==m_radMBComp["Tracker"].end()) {
            ...
            m_radMBComp["Tracker"][compName].SetBins(m_nBins, 0, p4);
            ...
            m_intMBComp["Tracker"][compName].SetBins(m_nBins, 0, p4);
          }

          // Fill
          m_radMBComp[trkName][compName].Fill(t2, iMap.second.radiation);
          m_intMBComp[trkName][compName].Fill(t2, iMap.second.interaction);

          m_radMBComp["Tracker"][compName].Fill(t2, iMap.second.radiation);
          m_intMBComp["Tracker"][compName].Fill(t2, iMap.second.interaction);
        }
      }
...
bool AnalyzerMatBudget::visualize(RootWSite& webSite)
{
  ...
  for (int iTrk=0; iTrk<=m_trackers.size(); iTrk++) {
    ...
    m_radMB[trkName]["Total"].SetXTitle("#theta");
    ...
    m_intMB[trkName]["Total"].SetXTitle("#theta");
    ...
    radContainer->GetXaxis()->SetTitle("#theta");
    ...
    intContainer->GetXaxis()->SetTitle("#theta");
    ...
    radCompStack->GetXaxis()->SetTitle("#theta");
    compLegend->Draw();
    ...
    intCompStack->GetXaxis()->SetTitle("#theta");
    compLegend->Draw();
    ...
    if (trkName!="Tracker") {
      ...
      m_hadronTotalHitsGraph[trkName].GetXaxis()->SetTitle("#theta");
      ...
      ranger->GetXaxis()->SetTitle("#theta");
...
void VisitorMatBudget::analyzeModuleMB(const DetectorModule& m)
{
  ...
  if ((m.maxZ()-m_matTrack.getOrigin().Z())>0) {
    ...
    if (m.checkTrackHits(m_matTrack.getOrigin(), direction, hitMaterial, hitType, hitPosition)) {
      ...
      // Fill material map [R,Z] -> X/X0
      if (m.getModuleCap().getRadiationLength()>0){

        m_radMap.Fill(hitZPos,hitRPos,m.getModuleCap().getRadiationLength());
        m_radMapCount.Fill(hitZPos,hitRPos);
      }
      if (m.getModuleCap().getInteractionLength()>0) {

        m_intMap.Fill(hitZPos,hitRPos,m.getModuleCap().getInteractionLength());
        m_intMapCount.Fill(hitZPos,hitRPos);
      }

      // Material budget: treat barrel & endcap modules separately
      if (m.subdet() == BARREL) {

        // Fill barrel container
        m_matBudget["Barrel"].radiation   += hitMaterial.radiation;
        m_matBudget["Barrel"].interaction += hitMaterial.interaction;
      }
      else if (m.subdet() == ENDCAP) {

        // Fill endcap container
        m_matBudget["Endcap"].radiation   += hitMaterial.radiation;
        m_matBudget["Endcap"].interaction += hitMaterial.interaction;
      }
      else {
        logWARNING("AnalyzerMatBudget::analyzeModuleMB -> incorrectly scaled material, unknown module type. Neither barrel or endcap");
      }

      // Material budget: components breakdown
      std::map<std::string, Material> components = m.getModuleCap().getComponentsRI();
      double theta = m_matTrack.getTheta();

      for (auto iComponent : components) {

        m_matBudget[iComponent.first].radiation   += iComponent.second.radiation / (m.subdet()==BARREL ? sin(theta + m.tiltAngle()) : cos(theta + m.tiltAngle() - M_PI/2));
        m_matBudget[iComponent.first].interaction += iComponent.second.interaction / (m.subdet()==BARREL ? sin(theta + m.tiltAngle()) : cos(theta + m.tiltAngle() - M_PI/2));
      }
...
//
void VisitorMatBudget::analyzeInactiveElement(std::string tag, const InactiveElement& e)
{
  ...
  if (e.checkTrackHits(m_matTrack.getOrigin(),direction, hitMaterial, hitPosition)) {
    ...
    // Fill maps
    if (e.getRadiationLength()>0) {
      m_radMap.Fill(hitZPos,hitRPos,e.getRadiationLength());
      m_radMapCount.Fill(hitZPos,hitRPos);
    }
    if (e.getInteractionLength()>0) {
      m_intMap.Fill(hitZPos,hitRPos,e.getInteractionLength());
      m_intMapCount.Fill(hitZPos,hitRPos);
    }

    m_matBudget[tag].radiation   += hitMaterial.radiation;
    m_matBudget[tag].interaction += hitMaterial.interaction;

  } // Hit found
}

double AnalyzerMatBudget::radtodeg (double theta) {
    double t2 = theta * 180 / M_PI;
    return t2;
  }