...
AnalyzerResolution::AnalyzerResolution(const Detector& detector) : AnalyzerUnit("AnalyzerResolution", detector),
 ...
 c_nBins(30)  // Default number of bins in histogram from eta=0  to max_eta_coverage)
{};
...
bool AnalyzerResolution::analyze()
{
  ...
  for (int iTrack = 0; iTrack < m_nTracks; iTrack++) {
    ...
    //c'est sale
    double theta = 0 + p3/m_nTracks*(iTrack+0.5);
    double eta = theta;
...
void AnalyzerResolution::preparePlot(std::vector<unique_ptr<TProfile>>& profHisArray, std::string varType, std::string scenario, const std::map<int, TrackCollection>& mapCollection)
{
  ...
  for (auto& col : mapCollection) {
    ...
    if (varType=="pT") {
      ...
      title = "p_{T} resolution versus #theta - "+scenario+" across #theta;#theta;#delta p_{T}/p_{T} [%]";
    }
    else if (varType=="p") {
      name  = "p_vs_eta"+any2str(momentum/Units::GeV);
      title = "p resolution versus #theta - "+scenario+" across #theta;#theta;#delta p/p [%]";
    }
    else if (varType=="d0") {
      name  = "d0_vs_eta"+any2str(momentum/Units::GeV);
      title = "Transverse impact parameter error - "+scenario+" across #theta;#theta;#delta d_{0} [#mum]";
    }
    else if (varType=="z0") {
      name  = "z0_vs_eta"+any2str(momentum/Units::GeV);
      title = "Longitudinal impact parameter error - "+scenario+" across #theta;#theta;#delta z_{0} [#mum]";
    }
    else if (varType=="phi0") {
      name  = "phi0_vs_eta"+any2str(momentum/Units::GeV);
      title = "Track azimuthal angle error - "+scenario+" across #theta;#theta;#delta #phi [deg]";
    }
    else if (varType=="cotgTheta") {
      name  = "cotgTh_vs_eta"+any2str(momentum/Units::GeV);
      title = "Track polar angle error - "+scenario+" across #theta;#theta;#delta ctg(#theta)";
    }
    else if (varType=="ctau") {
      name  = "ctau_vs_eta"+any2str(momentum/Units::GeV);
      title = "c#tau resolution - "+scenario+" across #theta;#theta;#delta c#tau [#mum]";
    }
    std::unique_ptr<TProfile> profHis(new TProfile(name.c_str(), title.c_str(), c_nBins, 0, p5));
    ...
    for (auto& track : col.second) {
      ...
      double tempabs = track->getTheta();
      double xVal = tempabs * 180 / M_PI;
      double yVal = 0;
